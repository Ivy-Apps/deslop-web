...
const tw = {
  ...baseTw,
  terminal: baseTw.window,
  hero: {
    dividerLine: baseTw.gradient.brandBorder,
    ambientRadial: landingEffects.ambientRadial,
    blurBlobLeft: landingEffects.blurBlobLeft,
    blurBlobRight: landingEffects.blurBlobRight,
    gridOverlay: landingEffects.gridOverlay,
    scanBeam: landingEffects.scanBeam,
    badgeOutline: `${baseTw.gradient.brandBadge} rounded-full p-px shadow-[0_0_40px_-6px_rgba(62,153,245,0.4),0_0_44px_-6px_rgba(92,61,245,0.45)]`,
    badgeDot: landingEffects.badgeDot,
    titleGradientBrandForward: baseTw.gradient.brandText,
    titleGradientBrandReverse: baseTw.gradient.brandTextReverse,
...
const display = {
  hero: `text-balance ${typeScale.display2xl}`,
  heroLine: `mt-2 md:mt-4 block ${typeScale.display2xl}`,
  sectionXl: `${typeScale.displayXl} mb-6 ${baseTw.text.primary}`,
  sectionLg: `${typeScale.displayLg} mb-6 ${baseTw.text.primary}`,
  sectionLgSpaced: `${typeScale.displayLg} mb-6 ${baseTw.text.primary}`,
  cta: `${typeScale.display2xl} mb-8 tracking-tight leading-[1.1]`,
} as const;

const gradientText = {
  heroLight: `block ${baseTw.gradient.lightText} drop-shadow-[0_0_48px_rgba(255,255,255,0.12)]`,
  heroFiller: baseTw.gradient.softLightText,
} as const;

const lead = {
  sectionMuted: `${textPresets.sectionLeadMuted} ${baseTw.text.muted}`,
  section: `${typeScale.bodyXl} ${baseTw.text.secondary} mb-8`,
  hero: `${textPresets.heroLead} ${baseTw.text.secondary} mb-10`,
  cta: `${textPresets.ctaLead} ${baseTw.text.muted} mb-12`,
} as const;

const heading = {
  integrationCard: `${typeScale.titleMd} mb-2`,
  pricingCard: `${typeScale.titleLg} mb-2`,
} as const;

const body = {
  smMuted: `${typeScale.bodySm} ${baseTw.text.subtle} max-w-xl mx-auto`,
  pricing: `${typeScale.bodyLg} ${baseTw.text.muted} mb-8`,
  list: `${textPresets.bodyList} ${baseTw.text.secondary}`,
  listLarge: `${textPresets.bodyListLg} text-zinc-200`,
  integrationMuted: `${typeScale.bodyMd} ${baseTw.text.muted}`,
  stepDownEngineTitle: `${typeScale.bodyMd} ${baseTw.text.primary} font-bold`,
  stepDownEngineMeta: `${typeScale.bodySm} ${baseTw.text.muted}`,
  stepDownBody: `${typeScale.bodyMd} ${baseTw.text.secondary}`,
} as const;

const code = {
  filenameHeader: `${textPresets.codeFilename} ${baseTw.text.muted}`,
  heroYaml: `${textPresets.codePanel} ${baseTw.text.subtle}`,
  lineNumber: `${textPresets.codeLineNumber} text-zinc-600`,
  windowTitle: `${textPresets.codeSm} ${baseTw.text.subtle} ml-2`,
  console: `space-y-4 ${typeScale.codeMd}`,
} as const;

export default function LandingView() {
  return (
    <div className={`${appText.shell} ${tw.bg.page}`}>
      <Navbar />
      <HeroSection />
      <BigThreeSection />
      <IntegrationSection />
...
function HeroVisualEffects(): ReactNode {
  return (
    <>
      <div
        className={`pointer-events-none absolute bottom-0 left-0 right-0 z-20 h-px ${tw.hero.dividerLine}`}
        aria-hidden
      />
      <div className={tw.hero.ambientRadial} aria-hidden />
      <motion.div
      ...
      />
      <motion.div
        className={tw.hero.blurBlobRight}
      ...
      <div
        ...
        aria-hidden
      >
        <div className={`absolute inset-0 ${tw.hero.gridOverlay}`} />
      </div>
      <div className={tw.hero.scanBeam} aria-hidden />
      <div
...
function HeroCodePreview(): ReactNode {
  return (
    ...
    >
      <div className={tw.hero.previewGlowBottom} aria-hidden />
      <div className={tw.hero.previewGlowTop} aria-hidden />
      <div className={tw.hero.previewCard}>
    ...
    >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
        ...
        <div className="p-7 bg-white/5 rounded-2xl border border-white/10">
          ...
          <h4 className={heading.integrationCard}>CI Mode</h4>
          <p className={body.integrationMuted}>
...
function TeamPlanCard(): ReactNode {
  return (
    <div className="bg-white/[0.02] border border-white/5 rounded-3xl p-10 group">
      <h3 className={heading.pricingCard}>CI / Team License</h3>
      <div className="flex items-baseline gap-1 mb-6">
    ...
    >
      ...
      <div className="bg-white/5 border border-white/10 rounded-2xl p-7">
        <div className="flex items-center gap-4 mb-4">
          <div className={tw.stepDown.iconCircle}>
            <Shield className={tw.stepDown.icon} />
          </div>
          <div>
            <p className={body.stepDownEngineTitle}>Haskell Engine</p>
            <p className={body.stepDownEngineMeta}>
              Purely Functional | AST-Safe
            </p>
          </div>
        </div>
        <p className={body.stepDownBody}>
